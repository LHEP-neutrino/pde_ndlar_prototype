...
def process_tpc_from_line_LTcrossing(
    ...
    det_positions_local,
    z_safe=47.5,
    z_cross_tol=1,  # changed to 1 cm tolerance, because with 0.5 there were some issues
):
    bounds = tpc_bounds[tpc_idx]
    lower, upper = np.array(bounds[0]), np.array(bounds[1])
...
def worker_process_file(
    *,
    ...
    # Global accumulators (initialized after first accepted event)
    PE_meas_tot = None
    ...
    with h5flow.data.H5FlowDataManager(hdf5_path, "r") as f:
        for event in p_events:
            ...
            # ---------------------------------
            # Initialize global accumulators once (first accepted event)
            # ---------------------------------
    ...
    # If everything got skipped, avoid crashing on None accumulators
    if PE_meas_tot is None:  # ADDED
        print(
            f"[rank worker] No events processed successfully for '{hdf5_name}'. "
            f"Skipped {skipped_events_ransac} events due to RANSAC failure."
        )  # ADDED
        if skipped_event_ids_ransac:  # ADDED
            print(f"[rank worker] RANSAC failed for events: {sorted(set(skipped_event_ids_ransac))}")  # ADDED
        return  # ADDED
    ...
    print(f"[rank worker] Skipped {skipped_events_ransac} events due to RANSAC failure.")  # ADDED
    if skipped_event_ids_ransac:  # ADDED
        print(f"[rank worker] RANSAC failed for events: {sorted(set(skipped_event_ids_ransac))}")  # ADDED
